...
@section('title', __('regions.edit_region_title'))
...
@section('content')
    ...
    <h1 class="mt-1 text-xl font-semibold text-text">{{ __('regions.edit_region_title') }} — {{ $region->code }}</h1>

    <form method="POST" action="{{ route('admin.regions.update', $region) }}" class="mt-6 max-w-md space-y-4 rounded-2xl border border-border bg-surface p-6">
        @csrf
        @method('PUT')
        ...
        <div>
            ...
            <input
                type="text" name="name_fr" value="{{ old('name_fr', $region->getTranslation('name', 'fr')) }}" required
        ...
        <div>
            ...
            <input
                type="text" name="name_ar" dir="rtl" value="{{ old('name_ar', $region->getTranslation('name', 'ar')) }}" required
                class="mt-1 block w-full rounded-lg border border-border px-3 py-2 text-sm focus:border-primary focus:outline-none focus:ring-1 focus:ring-primary"
            >
        </div>

        <label class="flex items-center gap-2 text-sm text-text">
            <input type="checkbox" name="is_active" value="1" @checked($region->is_active) class="rounded border-border text-primary focus:ring-primary">
